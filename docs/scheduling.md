...
| `arcana-cadence-b75adb8d.timer` → `arcana-cadence-b75adb8d.service` | oneshot | **every minute** | the same cadence for a second Season 2 competition (`b75adb8d-3215-4e53-9e41-9afa118151e6`), with its own lock. Seats every active agent that holds no seat anywhere before it opens a tick, then opens none if the field is still empty |
| `arcana-pace.timer` → `arcana-pace.service` | oneshot | **every minute** | **each agent decides on ITS OWN cadence** (`agents.cadence_seconds`, migration 0054). One unit for the whole platform, no competition id: it asks agent-service which agents are due — active, seated, not a fixture, not human-managed, and longer than their own interval since their LAST RECORDED DECISION — and asks the engine for one decision each against a single pool snapshot. **The only caller of the decision engine**; the cadence units mark the window the leaderboard reads and no longer decide for anybody, because two callers would double every agent's decisions and its fees ([cadence.md](./cadence.md)) |
...
| `arcana-capital-watchdog.timer` → `arcana-capital-watchdog.service` | oneshot | **every 5 minutes** | shouts if a borrowing position is under its health-factor floor, under 1.2 (near liquidation), or has not been read for ten minutes. An unchanged set of findings is not re-sent. See [capital.md](./capital.md) |
...
| `arcana-guard-watchdog.timer` → `arcana-guard-watchdog.service` | oneshot | **every 2 minutes** | is the position guard still watching? Reads the HEARTBEAT the guard writes on every scan — a fact only the guard can produce, unlike a `pgrep` that can match its own command line — alongside `systemctl is-active`, because the combination distinguishes "not running" from "running and stuck" from "scanning and failing". Far more often than the other watchdogs because it watches something that reports every fifteen seconds |
| `arcana-scoring-job.timer` → `arcana-scoring-job.service` | oneshot | **daily 23:30 UTC** | run the ARCANA Score batch, after the tick |
| `arcana-thesis-resolve.timer` → `arcana-thesis-resolve.service` | oneshot | **hourly, on the hour** (UTC) | resolves every published thesis whose own `resolves_at` has passed. Hourly rather than daily because each creator picks their own deadline, and a daily sweep would leave a claim that fell due at 00:05 reading "pending" for most of a day; `Persistent=true`, so a deadline that passed while the host was down is answered on the next boot rather than skipped — the measurement window is fixed by `created_at` and `resolves_at`, so a late run reads exactly the data an on-time one would have. A oneshot over HTTP through `arca-job.sh`, not a binary: the measurement reads the market index, and agent-service owns the single definition of what the market did on a tick. `flock`, because the job is idempotent but not free — two overlapping runs would both measure every due thesis ([theses.md](./theses.md)) |
...
| `arcana-anchor-signer.service` | long-running | always | the anchoring signer: its own Linux user `arcana-anchor`, its own key (not derived from the agent seed), one transaction shape — a zero-value self-send carrying a Merkle root. Capped fee, gas and 96 signatures a day; never broadcasts (port 8087 — [anchoring.md](./anchoring.md)) |
| `arcana-anchor.timer` → `arcana-anchor.service` | oneshot | **every 15 minutes** | write the Merkle root of newly sealed decision commitments on chain, only when there is something new. Settles a pending anchor before starting another; gas is a platform cost recorded per anchor ([anchoring.md](./anchoring.md)) |
| `arcana-error-watch.timer` → `arcana-error-watch.service` | oneshot | **every 10 minutes** | read the agent, arca, marketplace and web journals for new server errors and alert. A service answering 500s is still an active unit, so `OnFailure=` cannot see it ([alerting.md](./alerting.md)) |
...
not elapsed, and on one nobody has entered. Before each tick it calls
`POST /internal/v1/competitions/:id/participants/reconcile`, which seats every
ACTIVE agent holding no seat in any open competition: an active agent nothing
calls is a silent fault, and the caller is the only thing placed to notice it
has nobody to call.
...
`b75adb8d-3215-4e53-9e41-9afa118151e6` — the one a newly activated agent is
seated in, and the one that reconciles the field before every tick for the
agents that arrived by some other path.
